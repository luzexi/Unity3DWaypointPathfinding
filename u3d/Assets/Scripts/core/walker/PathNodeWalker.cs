...
[AddComponentMenu("PathNode/Walker")]
public class PathNodeWalker : MonoBehaviour
{
    ...
    private List<PathNode> pathContainer = new List<PathNode>();
    ...
    public virtual Vector3[] GetPathPoints(bool _local)
    {
        Vector3[] vec = new Vector3[pathContainer.Count];
        for(int i = 0 ; i<pathContainer.Count ; i++)
        {
            vec[i] = pathContainer[i].mPosition;
        }
        return vec;
    }

    public virtual Quaternion GetPointsRotation(int _index)
    {
        return pathContainer[_index].mRotation;
    }

    public virtual Vector3 GetPointsPosition(int _index)
    {
        return pathContainer[_index].mPosition;
    }
    ...
    public virtual bool IsPointsNull()
    {
        if(pathContainer == null) return true;
        return false;
    }
    ...
    /// <summary>
    public void StartMove(List<PathNode> newPath)
    {
    ...
    private void Initialize(int startAt = 0)
    {
        ...
        for (int i = events.Count; i <= GetPointsCount() - 1; i++)
    ...
    private void OnWaypointRotation()
    {
        ...
        lookPoint = Mathf.Clamp(currentPoint, 0, GetPointsCount());

        if (!tween.IsInitialized() || tween.IsComplete())
        {
            ApplyWaypointRotation(GetPointsRotation(lookPoint));
            return;
        }
        ...
        if (moveToPath)
        {
            ...
            ApplyWaypointRotation(Quaternion.Lerp(originRot, GetPointsRotation(currentPoint), currentPerc));
            return;
        }
        ...
        ApplyWaypointRotation(Quaternion.Lerp(GetPointsRotation(lookPoint), GetPointsRotation(reverse ? lookPoint - 1 : lookPoint + 1), currentPerc));
    }
    ...
    public void ResetToStart()
    {
        ...
        if (!IsPointsNull())
        {
            transform.position =GetPointsPosition(currentPoint) + new Vector3(0, sizeToAdd, 0);
        }
    }
